<?php
// classes/Artwork.php - Artwork Management Class  

require_once APP_PATH . '/config/database.php';
require_once APP_PATH . '/utils/Security.php';
// Assuming logActivity in Artwork class needs Auth class implicitly for user ID
// Or user ID can come from session directly in the calling API handler.
// For now, let's just make sure Security and Database are included.
...
class Artwork {
    ...
    public function update($id, $data) {
        try {
            ...
            if ($stmt->execute($params)) {
                ...
                $this->logActivity($_SESSION['user_id'] ?? null, 'artwork_updated', 'artworks', $id, 
                                 $current['data'], $data);
    ...
    public function delete($id) {
        try {
            ...
            if ($stmt->execute([$id])) {
                // Assuming $_SESSION['user_id'] is available for logging in the API endpoint
                $this->logActivity($_SESSION['user_id'] ?? null, 'artwork_deleted', 'artworks', $id, 
                                 $current['data'], null);
    ...
    public function submit($artwork_id, $artist_id, $notes = '') {
        try {
            ...
            if ($stmt->execute([$artwork_id, $artist_id, $notes, $notes])) {
                ...
                // This part would ideally update the status in the Artwork class itself,
                // but for now, we'll keep it as a direct query.
                $update_artwork_query = "UPDATE artworks SET status = 'submitted' WHERE id = ?";
                $update_artwork_stmt = $this->conn->prepare($update_artwork_query);
                $update_artwork_stmt->execute([$artwork_id]);
